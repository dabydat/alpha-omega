# AGENTS.md — alpha-omega (universal gate / SOP)

> Read by opencode, Codex, Copilot, Gemini. This is the universal gate.
> Claude uses `.claude/CLAUDE.md`; Cursor uses `.cursor/rules/nsp-rev.mdc`.

## Read config first

`config.json` is the control panel:
- `ai_org` — which gate is open (only that one is read).
...
- **MEMORY** — two locations, one immutable:
  ...
  - `<project>/<ai_org>/memory/*.md` → **YOUR state.** Read it there first, write it
    there on consolidation. It stays where `install.sh` put it; it never moves.

  A project state file carrying `TEMPLATE — DO NOT MODIFY` means the install
  copied too much. Strip it to the title + pointer and write your state. Don't relocate it.
...
4. `<ai_org>/memory/STATE.md` — your live state. Read it before planning, not after.
